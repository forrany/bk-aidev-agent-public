...
class ChatCompletionAgent(BaseModel):
    ...
    def execute(self, execute_kwargs: ExecuteKwargs) -> dict | Generator[str, None, None] | str:
        ...
        if execute_kwargs.run_agent or self.is_run_by_agent():
            return self._execute_by_agent(messages, execute_kwargs=execute_kwargs)
        if self.callbacks:
    ...
    def _execute_by_agent(self, messages: list[BaseMessage], execute_kwargs: ExecuteKwargs=None):
        if not messages:
        ...
        if execute_kwargs.stream:
            return agent_e.agent.stream_standard_event(
                agent_e,
                cfg,
                {"messages": messages,"execute_kwargs": execute_kwargs},
                timeout=self.agent_options.intent_recognition_options.heartbeats_interval,
            )
        else:
            loop = get_event_loop()
            result = loop.run_until_complete(agent_e.ainvoke({"messages": messages, "execute_kwargs": execute_kwargs}, cfg))
            return_data = {
