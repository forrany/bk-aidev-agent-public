...
# 角色与任务
你是一个结论提取专家，负责从大模型输出内容中识别并提取最终结论。
...
1. **如果包含最终结论：**
   - 提取核心结论，用清晰简洁的语言总结
   - 只返回总结后的结论内容，不加任何前缀、引导词或格式化符号
   - 确保结论是完整的句子或段落

2. **如果不包含最终结论（纯思考过程或无实质结论）：**
   - 返回：`抱歉，由于LLM指令遵从效果欠佳，尝试从思考内容中解析最终结论失败，请从思考内容中获取结论。`
   - 不要添加任何解释或额外内容

# 重要原则
- 严格区分思考过程与最终结论：思考过程描述推理路径，最终结论给出答案
- 工具调用失败也需总结：明确说明失败原因或当前状态
- 只返回要求的内容，不要有额外输出
"""

extract_conclusion_usr_prompt_template = env.from_string("""请分析以下大模型输出内容并提取最终结论：```{{last_chat_model_content}}```""")
