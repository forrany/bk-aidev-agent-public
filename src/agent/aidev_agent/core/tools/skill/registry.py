...
class SkillRegistry:
    ...
    支持**基于提供者**的可扩展性模型。每个 :class:`~SkillProvider`
    负责发现技能并获取其说明。注册表汇总来自所有已注册提供者的结果。

    向后兼容性
    ---------------------
    当 *sources* 包含纯 ``str`` 路径（原始 API）时，每个路径
    自动包装在 :class:`LocalSkillProvider` 中。这意味着
    ...
    def __init__(self, sources: list[Union[str, SkillProvider]]) -> None:
        self._providers: list[SkillProvider] = []
        self._skills: dict[str, SkillOptions] = {}
        self._skill_provider_map: dict[str, SkillProvider] = {}
        self._loaded = False

        for source in sources:
            if isinstance(source, str):
                # 将字符串源视为本地文件系统路径
                self._providers.append(LocalSkillProvider(source))
            else:
                # 直接的 SkillProvider 实例
    ...
    def ensure_loaded(self) -> None:
        ...
        skill_provider_map: dict[str, SkillProvider] = {}
