...
class FakeKnowledgeCompressor:
    ...
    def __call__(self, provided_chat_history, query, context, llm):
        self.calls += 1
...
class TestVariablesMiddleware:
    ...
    def test_tool_output_compression_structured_mode_uses_string_len(self):
        pipeline = MiddlewarePipeline()
        pipeline.use(BaseVariablesMiddleware())
        pipeline.use(ToolOutputCompressionMiddleware(tool_output_compress_thrd=10))
        pipeline.use(
            SpecialVariablesMiddleware(
                use_structured_response=True,
                use_general_knowledge_on_miss=False,
        ...
        messages = [
            ...
            ToolMessage(content="A" * 50, tool_call_id="call_1", name="t"),
        ]
        ...
        assert "观察结果：Z" in ctx.variables["agent_scratchpad"]
        # 不修改原始 state.messages
        assert messages[-1].content == "A" * 50
    ...
    def test_chat_history_compression_can_drop_chat_history(self):
        ...
        pipeline.use(ChatHistoryCompressionMiddleware())
    ...
    def test_token_compression_middleware_backcompat_wrapper(self):
        pipeline = MiddlewarePipeline()
        pipeline.use(BaseVariablesMiddleware())
        pipeline.use(
            SpecialVariablesMiddleware(
        ...
        pipeline.use(TokenCompressionMiddleware())
    ...
    def test_knowledge_compression_first_time(self):
        ...
        pipeline.use(KnowledgeCompressionMiddleware())
