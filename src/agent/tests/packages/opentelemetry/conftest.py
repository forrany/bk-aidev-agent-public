...
pytest.importorskip("opentelemetry", reason="install aidev-agent[opentelemetry] to run OTel tests")
...
try:
    ...
    pytest.skip(
        f"opentelemetry extras 不完整或版本不一致: {exc}; 请重新执行 `uv pip install aidev-agent[opentelemetry]`",
