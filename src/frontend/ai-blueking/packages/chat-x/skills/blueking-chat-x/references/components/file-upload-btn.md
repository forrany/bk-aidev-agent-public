...
输入区重构后，`ChatInput` 的上传入口改为左下角 [AddMenuBtn](/components/input/add-menu-btn) 聚合菜单里的「文件」「图片」项（内部走自持的隐藏 `input[type=file]`）。本组件仍保留在源码中并可用于自建输入区，但**不会**出现在 `ChatInput` 的默认布局里，也**未从包入口导出**——文档站示例经相对路径引入。
:::
...
仅在**自建输入区**时使用。若使用 `ChatInput`，上传能力由 `supportUpload`（默认 `true`）开启，入口是 + 号菜单里的「文件」「图片」项、拖拽与粘贴，无需再挂本按钮。
