...
aiSummary: >
  useFullScreen 将目标元素（或 document.documentElement）以浏览器原生全屏展示。
  模块加载时一次性嗅探 requestFullscreen / webkitRequestFullscreen，返回 isSupported、只读 isFullScreen 与 enter/exit/toggle。
  监听 fullscreenchange 同步 ESC 等外部退出；ChatContainer 侧栏全屏按钮使用此 composable。
relatedComponents:
  - slug: chat-container
...
<script lang="ts" setup>
  ...
  import { useFullScreen } from '../../src/composables/use-full-screen';
...
<div class="demo">
  <div
    ref="panelRef"
    style="padding: 16px; margin-bottom: 8px; border: 1px solid #eaebf0; border-radius: 4px; background: #f5f7fa;"
  >
    <p style="margin: 0; font-size: 13px;">可全屏展示的面板内容</p>
  </div>
  <div style="display: flex; gap: 8px; align-items: center; font-size: 12px;">
    <button
      :disabled="!isSupported"
      @click="enter"
      style="padding: 4px 12px; border: 1px solid #3a84ff; border-radius: 4px; background: #3a84ff; color: #fff; cursor: pointer;"
    >进入全屏</button>
    <button
      :disabled="!isFullScreen"
      @click="exit"
      style="padding: 4px 12px; border: 1px solid #dcdee5; border-radius: 4px; background: #fff; cursor: pointer;"
    >退出全屏</button>
    <span style="color: #979ba5;">{{ isFullScreen ? '全屏' : '窗口' }}{{ isSupported ? '' : '（当前环境不支持全屏）' }}</span>
  </div>
</div>
