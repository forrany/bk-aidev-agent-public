---
name: useInputMention
slug: use-input-mention
category: composable
description: 通过 provide/inject 把「资源插入输入框」的能力开放给任意深度的后代组件。
aiSummary: >
  useInputMentionProvider 由持有输入框的容器（ChatContainer）提供 insertMention，
  useInputMentionConsumer 在任意后代取出；无 Provider（只读 / 分享态）时返回 undefined，调用方据此隐藏「引用」入口。
  消息区文件卡片与侧栏产物面板即用它把文件以标签形式追加进输入框。
  源码位置：src/composables/use-input-mention.ts。
relatedComponents:
  - slug: chat-container
    relation: 根容器提供 insertMention，内部转发给 ChatInput 实例
  - slug: chat-input
    relation: 实际执行插入的组件，expose insertMention
  - slug: file-artifact-panel
    relation: 侧栏产物面板据此展示「引用」按钮
  - slug: assistant-message
    relation: 消息内文件卡片据此展示「引用」按钮
sinceVersion: 0.0.51
---
