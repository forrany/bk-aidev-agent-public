---
name: MentionTag 资源标签
slug: mention-tag
kind: component
domain: rendering
description: 单个资源标签，带类型图标、描述气泡与会话产物点击预览。
aiSummary: >
  MentionTag 渲染「图标 + 蓝色名称」的内联资源标签：节点数据全部落在 data-tag-* 属性上，
  既是编辑器 void 节点的识别依据，也让消息里复制的标签能原样还原；
  description 非空时 hover / 点击弹出「类型：名称 + 描述」气泡；type 为 artifact 且存在产物预览上下文时点击打开侧栏预览。
  源码位置：src/components/mention/mention-tag.vue（气泡见 mention-popover.vue 与 create-mention-tippy.ts）。
relatedComponents:
  - slug: mention-text
    relation: 按文档结构批量渲染标签与文本
  - slug: ai-slash-input
    relation: 编辑器内的 tag 节点由本组件渲染
  - slug: resource-icon
    relation: 标签左侧图标
...
<script lang="ts" setup>
  import MentionTagComp from '../../../src/components/mention/mention-tag.vue';

  const tags = [
    { label: '翻译', value: 'translate', type: 'skill', description: '把选中的文本翻译成目标语言' },
    { label: 'database-server', value: 'database-server', type: 'mcp', description: '' },
    { label: '天气查询', value: 'weather', type: 'tool', description: '按城市查询实时天气' },
    { label: 'API 接口文档', value: 'kb-api', type: 'knowledgebase', description: '' },
    { label: '巡检报告.pdf', value: 'output-1', type: 'artifact', description: '' },
  ];
</script>
...
<div class="demo" style="display: flex; flex-wrap: wrap; gap: 16px; align-items: center;">
  <MentionTagComp
    v-for="tag in tags"
    :key="tag.value"
