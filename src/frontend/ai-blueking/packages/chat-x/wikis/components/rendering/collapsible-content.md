---
name: CollapsibleContent 折叠内容
slug: collapsible-content
kind: component
domain: rendering
description: 内容超过阈值高度时折叠，并在下方展示「显示更多 / 收起」。
aiSummary: >
  CollapsibleContent 用 ResizeObserver 持续测量插槽真实高度，超过 maxHeight（默认 200px）才折叠并显示切换按钮；
  折叠通过外层 max-height + overflow hidden 实现，展开态可由 v-model:expanded 受控，便于「全部展开」这类批量操作。
  源码位置：src/components/chat-content/collapsible-content/collapsible-content.vue。
relatedComponents:
  - slug: user-message
    relation: 用户消息正文超过 200px 时折叠
...
<script lang="ts" setup>
  import { ref } from 'vue';
  import CollapsibleContentComp from '../../../src/components/chat-content/collapsible-content/collapsible-content.vue';

  const expanded = ref(false);
  const longText = Array.from({ length: 14 }, (_, index) => `第 ${index + 1} 行内容，用于演示折叠与展开的高度变化。`).join('\n');
</script>
...
<div class="demo">
  <div style="margin-bottom: 8px; font-size: 12px; color: #979ba5;">
    expanded = {{ expanded }}
    <button style="margin-left: 8px; padding: 2px 8px; font-size: 12px;" @click="expanded = !expanded">外部切换</button>
  </div>
  <CollapsibleContentComp
    v-model:expanded="expanded"
    :max-height="120"
  >
    <div style="white-space: pre-wrap; font-size: 12px; line-height: 20px; color: #4d4f56;">{{ longText }}</div>
  </CollapsibleContentComp>
</div>
