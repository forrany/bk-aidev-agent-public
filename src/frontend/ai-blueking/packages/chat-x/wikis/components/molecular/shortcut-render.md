...
category: molecular
description: >-
  快捷指令表单渲染组件，将 `Shortcut.components` 配置自动渲染为可交互表单（基于 bkui-vue 的 `Form`），支持 8
  种控件类型、两列网格布局、必填校验和内置提交/取消操作。
aiSummary: >
  ShortcutRender 将 Shortcut.components 声明渲染为 bkui-vue Form，支持多种控件与两列网格、必填校验及提交/取消。
  通常由 ChatContainer 等布局挂载在输入区旁，formModel 与快捷指令回填联动。
  与 Shortcut、ShortcutBtn 等共享同一套快捷指令数据模型。
relatedComponents:
  - slug: shortcut-btn
...
<script lang="ts" setup>
  import ShortcutRenderComp from '../../../src/components/ai-shortcut/shortcut-render/shortcut-render.vue'
  ...
  const allTypesComponents = [
    {
      ...
      key: 'language',
      name: '编程语言',
      placeholder: '如：TypeScript、Python',
    },
    {
      type: 'number',
      key: 'maxLines',
    ...
    },
    {
      ...
      options: [
        ...
        { label: 'TypeScript 类型', value: 'types' },
      ],
    },
    {
      type: 'switcher',
