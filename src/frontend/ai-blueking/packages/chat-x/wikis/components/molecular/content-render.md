...
category: molecular
description: >-
  消息内容渲染分发组件，根据 `content` 的 JavaScript 类型自动选择渲染方式：字符串 →
  `MarkdownContent`（Markdown 渲染），数组 → `ReferenceContent`（引用列表）。
aiSummary: >
  ContentRender 按消息 content 类型分发：字符串走 Markdown 管线，数组可走引用文档等；可扩展自定义 slot。
  主要 props 为 content、status，与 MessageRender、AssistantMessage 等配合使用。
  内置与 MarkdownContent、ReferenceContent 等原子/分子组件衔接。
relatedComponents:
