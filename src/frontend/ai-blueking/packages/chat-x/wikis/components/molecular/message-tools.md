...
category: molecular
description: >-
  AI 消息的操作工具栏组件，由**左侧消息工具区**和**右侧更新工具区**两部分组成，中间以分隔线分隔。仅 `like` / `unlike`
  按钮会弹出反馈表单（`UserFeedback`），其余按钮直接触发 `onAction`。
aiSummary: >
  MessageTools 渲染 AI 消息底部工具区：左侧消息工具（复制、引用、重建等），右侧更新工具；点赞/踩会弹出 UserFeedback 收集原因。
  工具 id 为 delete 时用 DeleteTool 替代普通 ToolBtn 做二次确认；其余操作通过 onAction 异步解析或直出。
relatedComponents:
  - slug: tool-btn
