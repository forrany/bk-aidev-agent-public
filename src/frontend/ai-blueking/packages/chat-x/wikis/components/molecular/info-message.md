...
aiSummary: >
  InfoMessage 以居中虚线分隔条展示系统提示、会话重置、时间戳等非对话信息；content 可为字符串或字符串数组。
  由 MessageRender 在 info 角色下渲染，不承载用户与助手轮次内容。
relatedComponents:
  - slug: message-render
...
- [MessageRender](./message-render.md) — info 角色由其实例化
- [MessageContainer](./message-container.md) — 列表内嵌时的外层容器
