...
<script lang="ts" setup>
  import ToolcallRenderComp from '../../../src/components/tool-call/toolcall-render/toolcall-render.vue'

  // 进行中 —— 标题显示「正在调用」并带渐变闪动，无状态段与折叠箭头
  const pendingToolCall = {
    id: 'call_1',
    type: 'function',
    function: {
      name: 'search_documents',
      arguments: JSON.stringify({ query: 'Vue 3 Composition API', limit: 10 }),
      description: '在知识库中搜索与查询词相关的文档资料',
    },
  };
  ...
  const successToolCall = {
    ...
    toolMessage: {
      role: 'tool',
      content: JSON.stringify({ city: '北京', temperature: 22, weather: '晴', humidity: 45, wind: '东北风 3 级' }),
      status: 'complete',
      duration: 1200,
      toolCallId: 'call_2',
    },
  ...
  const errorToolCall = {
    ...
    function: {
      name: 'execute_sql',
      arguments: JSON.stringify({ sql: 'SELECT * FROM users WHERE id = 1' }),
      description: '在数据库中执行 SQL 查询',
    },
    toolMessage: {
      role: 'tool',
      content: '',
      error: 'Connection timeout: database is unreachable',
      status: 'error',
      duration: 5000,
      toolCallId: 'call_3',
    },
  ...
  const mcpToolCall = {
    ...
    function: {
      type: 'mcp',
      name: 'query_table',
      arguments: JSON.stringify({ table: 'events', filters: { level: 'ERROR' }, limit: 50 }),
      description: '通过 MCP 协议查询蓝鲸数据平台中的事件数据',
      mcpName: 'bk-data-server',
    },
    toolMessage: {
      role: 'tool',
      content: JSON.stringify({ total: 12, items: [{ id: 1, level: 'ERROR', message: 'Service unavailable' }] }),
      status: 'complete',
      duration: 830,
      toolCallId: 'call_4',
    },
  ...
  // Skill 加载 —— 头部前缀为「读取 Skill」
  const skillToolCall = {
    id: 'call_6',
    type: 'function',
    function: {
      type: 'skill',
      name: 'bkui-vue-components',
      arguments: JSON.stringify({ topic: 'Table 表格' }),
      description: '加载 bkui-vue 组件库速查 Skill',
    },
    toolMessage: {
      role: 'tool',
      content: 'Skill 已加载，共 32 个组件条目',
      status: 'complete',
      duration: 120,
      toolCallId: 'call_6',
    },
  ...
  // 无 description —— 详情面板"描述"区块为空
  const noDescToolCall = {
    id: 'call_5',
    type: 'function',
    function: {
      name: 'list_files',
    ...
    toolMessage: {
      role: 'tool',
      content: JSON.stringify(['file1.txt', 'file2.txt', 'dir1/']),
      status: 'complete',
      duration: 45,
      toolCallId: 'call_5',
    },
...
<div class="demo">
  <ToolcallRenderComp :tool-call="successToolCall" status="complete" />
</div>
...
<div class="demo">
  <div style="display: flex; flex-direction: column; gap: 12px;">
    <ToolcallRenderComp :tool-call="pendingToolCall" status="pending" />
    <ToolcallRenderComp :tool-call="successToolCall" status="complete" />
    <ToolcallRenderComp :tool-call="errorToolCall" status="error" />
  </div>
</div>
...
<div class="demo">
  <div style="display: flex; flex-direction: column; gap: 12px;">
    <ToolcallRenderComp :tool-call="successToolCall" status="complete" />
    <ToolcallRenderComp :tool-call="mcpToolCall" status="complete" />
    <ToolcallRenderComp :tool-call="skillToolCall" status="complete" />
  </div>
</div>
...
<div class="demo">
  <ToolcallRenderComp :tool-call="noDescToolCall" status="complete" />
</div>
