...
aiSummary: >
  ShortcutBtns 横向展示快捷指令按钮组，按容器宽度动态计算可见数量，溢出收入「更多」下拉。
  默认由 ChatInput 嵌入附件槽；选中含 components 的指令后需配合 ShortcutRender 展示表单。
  通过 selectShortcut 等与外层同步当前快捷指令。
relatedComponents:
  - slug: shortcut-btn
...
<script lang="ts" setup>
      ...
      components: [
        {
          ...
          key: 'language',
          name: '编程语言',
          props: { placeholder: '如：JavaScript、Python' },
        },
        {
          type: 'textarea',
          key: 'description',
  ...
  const manyShortcuts = Array.from({ length: 12 }, (_, i) => ({
    id: `cmd-${i + 1}`,
    name: `指令 ${i + 1}`,
  }));

  const handleSelectShortcut = (shortcut) => {
    alert(`选择了：${shortcut.name}`);
  };
</script>
...
<div class="demo">
  <ShortcutBtns :shortcuts="basicShortcuts" @select-shortcut="handleSelectShortcut" />
</div>
...
<div class="demo">
  <ShortcutBtns :shortcuts="descShortcuts" @select-shortcut="handleSelectShortcut" />
</div>
...
<div class="demo">
  <ShortcutBtns :shortcuts="formShortcuts" @select-shortcut="handleSelectShortcut" />
  <p style="margin-top: 8px; font-size: 12px; color: #979ba5;">点击上方按钮触发 select-shortcut 事件（表单弹窗需配合 ShortcutRender）</p>
</div>
...
<div class="demo">
  <ShortcutBtns :shortcuts="manyShortcuts" @select-shortcut="handleSelectShortcut" />
</div>
