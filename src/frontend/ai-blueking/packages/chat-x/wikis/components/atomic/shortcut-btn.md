...
aiSummary: >
  ShortcutBtn 渲染单个快捷指令入口，封装图标解析与「按钮 / 下拉菜单项」两种布局，并暴露根元素供父级测量。
  由 ShortcutBtns、AiSelection、ChatInput 内部复用；含 components 的快捷指令可无默认图标。
  通过 click 事件传出当前 Shortcut，append 槽常用于关闭按钮。
relatedComponents:
  - slug: shortcut-btns
