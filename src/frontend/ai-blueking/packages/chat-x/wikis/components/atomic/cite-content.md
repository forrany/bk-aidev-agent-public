...
aiSummary: >
  CiteContent 在输入区上方以紧凑条带展示被引用文本片段，单行截断并可选关闭按钮移除引用。
  常与 ChatInput 的引用预览区配合，不执行 HTML 渲染。
relatedComponents:
  - slug: chat-input
...
<script lang="ts" setup>
  import { ref } from 'vue';
  import CiteContent from '../../../src/components/chat-content/cite-content/cite-content.vue'

  const showCite = ref(true);
  const handleClose = (content) => {
    showCite.value = false;
    console.log('关闭引用:', content);
  };
  const resetCite = () => { showCite.value = true; };
</script>
...
<div class="demo" style="max-width: 400px;">
  <CiteContent
    content="这是一段非常长的引用文本内容，当文本超出容器宽度时，会自动以省略号的形式截断显示，避免影响整体布局。"
    :on-close="() => {}"
  />
</div>
