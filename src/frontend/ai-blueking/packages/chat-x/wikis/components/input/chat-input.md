...
kind: component
domain: input
description: 聊天输入区，组合富文本输入、统一菜单（@ / \ 与 + 号）、快捷指令、附件、引用、发送/停止等交互。
aiSummary: >
  聊天输入区，组合富文本输入、统一菜单、快捷指令、附件、引用、发送/停止等交互。
  菜单数据由单一 menuSources 提供，按 type 分发到 `/` `@` `\` 与左下角 + 号四种触发方式。
  源码位置：src/components/chat-input/chat-input.vue。
relatedComponents:
  - slug: ai-slash-input
    relation: 内部富文本编辑区，负责触发符识别与标签插入
  - slug: input-menu-panel
    relation: 输入框正上方的统一菜单面板
  - slug: add-menu-btn
    relation: 左下角 + 号，唤起聚合菜单
  - slug: mention-tag
    relation: 菜单选中的资源以标签形式嵌入编辑器
  - slug: model-selector
...
<script lang="ts" setup>
  ...
  const menuSources = [
    ...
    { id: 'kb-spec', type: 'knowledgebase', name: '开发规范手册' },
...
- 在未满的前提下：空文件、单文件大小 **`>= MAX_UPLOAD_FILE_SIZE`（20MB，即 `20 * 1024 * 1024` 字节）** 会被跳过并弹出超大小/个数提示。与已有文件重复的项只去重、不弹这条误导文案。
