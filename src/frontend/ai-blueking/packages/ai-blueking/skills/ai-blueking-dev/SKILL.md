...
- 欢迎区 `#welcome` 插槽；Header `#headerLeft` / `#headerActions` 插槽；消息工具栏 `messageTools` / `updateTools` 扩展与 `agent-action` / `confirm-share(source)`；Claw 智能体（`agentType === 'claw'`）自动隐藏编辑/删除/重新生成
...
| 只需聊天区域，嵌入到自定义布局中               | ChatBot 独立模式       | 轻量、灵活；Header / 侧栏开关需业务自建 |
...
> **嵌入模式不是开箱即用的完整工作台**：`ChatBot` 只有聊天区。会话标题、侧栏展开/收起必须由业务 Header 提供（`v-model:asideCollapsed` + `CollapsedAsideIcon`）。可运行样例：`packages/ai-blueking/playground/views/EmbeddedHeaderView.vue`（标题栏「查看源码」可复制最小接入代码）；生产级实现：`publish-template/src/views/ChatWindow.vue`。详见 [集成模式 · 嵌入式业务 Header](references/integration-patterns.md#嵌入式-chatbot业务-header--侧栏开关)。
...
### 嵌入式 ChatBot 没有 Header / 侧栏开关

浮窗（`AIBlueking`）的展开/收起在 `AIHeader`。嵌入式 `ChatBot` **不提供**该按钮；不传 `asideCollapsed` 时内部自持（默认折叠），业务无法从外部切换。必须：

1. 自己画 Header（左侧会话名，右侧开关）
2. `v-model:asideCollapsed` 绑定（只写 `:aside-collapsed` 会让文件卡片 / `addCustomTab` 的内部展开失效）
3. 图标复用 `@blueking/chat-x` 的 `CollapsedAsideIcon`（它是 VNode，模板里用 `cloneVNode` 包一层组件）
...
### `#headerActions` 插槽自定义右侧工具栏图标

`#headerActions` 插在 `.right-section` 内部：会话类操作（新增 / 历史 / 转人工）之后、窗口控制（压缩 / 关闭 / 侧栏）之前。与 `showHistoryIcon` 独立——隐藏内置历史时仍可只留自定义图标。
...
**Vue2 用法：** 与 `#headerLeft` 相同，使用包导出的 `h` 或 template scoped slot，插槽名为 `headerActions`。

**插槽链路**：`AIBlueking #headerActions` → `AIHeader #headerActions`，无 slot props。子节点带 `bkai-icon` 时套用 Header 工具栏 20px / hover 样式。

**右侧工具栏顺序**（对齐设计稿）：`新增` → `历史` → `转人工` → `#headerActions` → `压缩` → `关闭` → 竖线分隔 → `侧栏展开/收起`。

**约束**：
- 插槽内容应保持简洁（推荐单行），避免破坏 Header 高度（48px）和拖拽交互
- 插槽内容不可见时（如条件渲染）不影响 Header 布局
- 不要用 `#headerLeft` 塞工具栏图标，它不在 `.right-section` 内
...
- [集成模式与示例](references/integration-patterns.md) — 各场景完整代码示例、包导出、常见任务速查；含 [嵌入式业务 Header](references/integration-patterns.md#嵌入式-chatbot业务-header--侧栏开关)
