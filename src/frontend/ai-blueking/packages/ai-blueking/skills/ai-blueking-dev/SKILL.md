...
- 欢迎区 `#welcome` 插槽；消息工具栏 `messageTools` / `updateTools` 扩展与 `agent-action` / `confirm-share(source)`；Claw 智能体（`agentType === 'claw'`）自动隐藏编辑/删除/重新生成
...
**右侧工具栏顺序**（对齐设计稿）：`新增` → `历史` → `转人工` → `压缩` → `关闭` → 竖线分隔 → `侧栏展开/收起`。
