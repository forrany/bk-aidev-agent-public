...
// 发送消息（内部自动处理自动重命名；默认携带 selectedLlmCode）
...
ChatInput 支持文件上传功能，传入 `onUpload` 回调后自动显示上传按钮。`session.uploadFile` 会按 `agent.info.agentSdkVersion` 分流：`< 2.2.2rc25` 走旧 `upload/{fileName}/`，`≥ 2.2.2rc25` 走 `pv_files/upload/`。
