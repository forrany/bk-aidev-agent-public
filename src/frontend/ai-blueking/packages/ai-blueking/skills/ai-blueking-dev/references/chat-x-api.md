...
>
> **当前版本**：`@blueking/chat-x` `2.4.0`（含 HITL 中断消息、RenderMode、ModelSelector、字号主题、`#group` 插槽、文件产物预览、`MessageTime` 消息时间、通用文件上传、统一输入菜单 `menuSources` 等能力）。与 `@blueking/ai-blueking` `2.4.0` 配套。
