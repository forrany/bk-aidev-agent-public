...
业务侧 ``SessionManager`` 内部通过 ``self._client()`` 取 client（注入的 rm，未注入时回落全局单例）；
为避免单测真实 HTTP，本 conftest 把 ``SessionManager._client`` 替换为返回 ``MagicMock`` 的方法，
并替换模块内 ``resource_manager_factory()`` 工厂。测试可断言 ``client.api.xxx`` 调用，或通过
...
def mock_plugin_rm_client(monkeypatch):
    ...
    monkeypatch.setattr(agent_session, "resource_manager_factory", lambda: resource_manager_mock)
